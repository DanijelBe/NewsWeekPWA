...
<body class="text-gray-900">
    ...
    <main class="container mx-auto my-8 p-4 md:p-6 bg-white shadow-lg rounded-lg">
        <h2 class="text-2xl md:text-3xl font-semibold text-gray-800 mb-6 border-b-2 border-red-400 pb-2">Articles</h2>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php
			$category = htmlspecialchars($_GET['category']);
			include 'connect.php';
			$stmt = $conn->prepare("SELECT id, title, category, date, image FROM articles WHERE category = ? ORDER BY date DESC");
